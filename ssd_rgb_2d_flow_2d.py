...
resultsPath="./results/ssd_rbg_2d_2d_"+OF
resultsPath_merge="./results/merge_ssd_rbg_2d_2d_"+OF
...
model_flow, predictor_sizes_flow = ssd_300(image_size=(img_height, img_width, img_channels),
                n_classes=n_classes,
				mode='training',
                l2_regularization=0.00004,#0.0005,
                scales=scales,
...
weights_path = './weights/ssd/vgg-16_ssd-fcn_ILSVRC-CLS-LOC.h5'

if load_prev_model==1:
    model_rgb.load_weights('./weights/ssd/FFssd300_final.h5',by_name=True)
if OF == 'liteFlow':
        model_flow.load_weights('./weights/ssd/FFssd300_liteflow_final.h5', by_name=True)
elif OF == 'Tvl1':
        model_flow.load_weights('./weights/ssd/FFssd300_Tvl1_final.h5', by_name=True)
else:
    model_rgb.load_weights(weights_path, by_name=True)
    model_flow.load_weights(weights_path, by_name=True)
    ...
    for clip_name_i in clip_name:
        ...
        if len(DirList_jpg)>0:
            ...
            for imgname in DirList_jpg:
                ...
                X_flow,X_flow_orig, y_flow,XY_scale=inference_single_image(filenames_flow,model_flow)
                ...
                vid_detect_only.write(np.uint8(img_merge))
                if (len(boxs)>0):
