...
class DataGenerator:
    ...
    def parse_csv(self,
        ...
        current_file_tmp=current_file.rsplit('_',1)
        clip_name=current_file_tmp[0]
        clip_num=0
        for i, box in enumerate(data):
            current_file_tmp=box[0].rsplit('_',1)[0]
            if not(clip_name==current_file_tmp):
                clip_num=clip_num+1
                clip_name=current_file_tmp
        ...
        if ret: # In case we want to return these
            return self.images, self.filenames, self.labels, self.image_ids,self.clip_num
        ...
        while True:
            ...
            if not (self.images is None):
                ...
                for filename in batch_filenames:
                    num_image = filename.split('.')[0].rsplit('\\', 1)[1]
                    filename_tmp = filename.split('.')[0].rsplit('\\', 1)[0]
                    num_image_int=int(num_image)
                    X_tmp=[]

                    for k in range(num_frame):
                            num_frame_k= ((num_image_int - k) if ( num_image_int - k>1) else 1)
                            filename_tmp_k = filename_tmp + '\\' + '{:0>5}'.format(str(num_frame_k)) + '.jpg'


                            if os.path.exists(filename_tmp_k):
                                with Image.open(filename_tmp_k) as image:
                                    X_tmp.append(np.array(image, dtype=np.uint8))
                            else:
                                X_tmp.append(X_tmp[-1])
                    batch_X.append(X_tmp)
                        ...
                        for j in range(len(batch_X[i])):
                            if not (self.labels is None):
                                ...
                                if ('inverse_transform' in returns) and ('return_inverter' in inspect.signature(transform).parameters):
                                    if (j == 0):
                                        batch_X[i][j], batch_y[i], inverse_transform = transform(batch_X[i][j], batch_y[i], return_inverter=True)
                                        inverse_transforms.append(inverse_transform)
                                    else:
                                        batch_X[i][j] = transform(batch_X[i][j],return_inverter=False)

                                else:
                                    if (j==0):
                                        batch_X[i][j], batch_y[i] = transform(batch_X[i][j], batch_y[i])
                                    else:
                                        batch_X[i][j] = transform(batch_X[i][j])
                                if batch_X[i][j] is None: # In case the transform failed to produce an output image, which is possible for some random transforms.
                                    batch_items_to_remove.append(i)
                                    if (j == 0):
                                        batch_inverse_transforms.append([])
                                    continue
                                ...
                                if ('inverse_transform' in returns) and ('return_inverter' in inspect.signature(transform).parameters):
                                    if (j == 0):
                                        batch_X[i][j], inverse_transform = transform(batch_X[i][j], return_inverter=True)
                                        inverse_transforms.append(inverse_transform)
                                    else:
                                        batch_X[i][j] = transform(batch_X[i][j], return_inverter=False)
                                else:
