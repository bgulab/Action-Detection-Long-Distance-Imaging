...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        '''
        print(self.neg_pos_ratio)
        self.neg_pos_ratio = K.constant(self.neg_pos_ratio,shape=(1,1))
        self.n_neg_min = K.constant(self.n_neg_min,shape=(1,1))
        self.alpha = K.constant(self.alpha,shape=(1,1))

        batch_size = K.shape(y_pred)[0] # Output dtype: K.int32
        n_boxes = K.shape(y_pred)[1] # Output dtype: K.int32, note that `n_boxes` in this context denotes the total number of boxes per image, not the number of boxes per cell.

        # 1: Compute the losses for class and box predictions for every box.

        classification_loss = K.cast(self.log_loss(y_true[:,:,:-12], y_pred[:,:,:-12]),dtype='float32') # Output shape: (batch_size, n_boxes)
        localization_loss = K.cast(self.smooth_L1_loss(y_true[:,:,-12:-8], y_pred[:,:,-12:-8]),dtype='float32') # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets.

        # Create masks for the positive and negative ground truth classes.
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        positives = K.cast(K.max(y_true[:,:,1:-12], axis=-1),dtype='float32') # Tensor of shape (batch_size, n_boxes)

        # Count the number of positive boxes (classes 1 to n) in y_true across the whole batch.
        n_positive = K.sum(positives)

        # Now mask all negative boxes and sum up the losses for the positive boxes PER batch item
        # (Keras loss functions must output one scalar loss value PER batch item, rather than just
        # one scalar for the entire batch, that's why we're not summing across all axes).
        pos_class_loss = K.sum(classification_loss * positives, axis=-1) # Tensor of shape (batch_size,)

        # Compute the classification loss for the negative default boxes (if there are any).

        # First, compute the classification loss for all negative boxes.
        neg_class_loss_all = classification_loss * negatives # Tensor of shape (batch_size, n_boxes)
        n_neg_losses = np.count_nonzero(neg_class_loss_all) # The number of non-zero loss entries in `neg_class_loss_all`
        #n_neg_losses = np.count_nonzero(neg_class_loss_all, dtype=K.int32) # The number of non-zero loss entries in `neg_class_loss_all`
        # What's the point of `n_neg_losses`? For the next step, which will be to compute which negative boxes enter the classification
        # loss, we don't just want to know how many negative ground truth boxes there are, but for how many of those there actually is
        # a positive (i.e. non-zero) loss. This is necessary because `K.nn.top-k()` in the function below will pick the top k boxes with
        ...
        # classification losses ARE actually zero though, this behavior might lead to `K.nn.top-k()` returning the indices of positive
        # boxes, leading to an incorrect negative classification loss computation, and hence an incorrect overall loss computation.
        # We therefore need to make sure that `n_negative_keep`, which assumes the role of the `k` argument in `K.nn.top-k()`,
        ...
        n_negative_keep = K.minimum(K.maximum(self.neg_pos_ratio * K.cast(n_positive,dtype='int32'), self.n_neg_min), n_neg_losses)
        ...
        def f2():
            ...
            neg_class_loss_all_1D = K.reshape(neg_class_loss_all, [-1]) # Tensor of shape (batch_size * n_boxes,)
            # ...and then we get the indices for the `n_negative_keep` boxes with the highest loss out of those...
            
            # ...and with these indices we'll create a mask...
            #if K.backend() != 'tensorflow':
            #        import mxnet as mx 
            #        values, indices = mx.symbol.topk(neg_class_loss_all_1D,ret_typ='both',
            #                              k=n_negative_keep
            #                              ) # We don't need them sorted.
            #        negatives_keep = mx.ndarray.scatter_nd(data=K.ones_like(indices, dtype='int32'),indices=K.expand_dims(indices, axis=1),
            #                               shape=K.shape(neg_class_loss_all_1D)) # Tensor of shape (batch_size * n_boxes,)
            #else:
            import tensorflow as tf
            values, indices = tf.nn.top_k(neg_class_loss_all_1D,k=n_negative_keep,
                                          sorted=False) # We don't need them sorted.
            negatives_keep = tf.scatter_nd(indices=K.expand_dims(indices, axis=1),
                                           updates=K.ones_like(indices, dtype='int32'),
                                           shape=K.shape(neg_class_loss_all_1D)) # Tensor of shape (batch_size * n_boxes,)
            negatives_keep = K.cast(K.reshape(negatives_keep, [batch_size, n_boxes]),dtype='float32') # Tensor of shape (batch_size, n_boxes)
            # ...and use it to keep only those boxes and mask all other classification losses
            neg_class_loss = K.sum(classification_loss * negatives_keep, axis=-1) # Tensor of shape (batch_size,)
            return neg_class_loss

        neg_class_loss = K.switch(K.equal(n_neg_losses, K.constant(0,shape=(1,1))), f1, f2)
        ...
        loc_loss = K.sum(localization_loss * positives, axis=-1) # Tensor of shape (batch_size,)

        # 4: Compute the total loss.

        total_loss = (class_loss + self.alpha * loc_loss) / K.maximum(1.0, n_positive) # In case `n_positive == 0`
        ...
        total_loss = total_loss * K.cast(batch_size,dtype='float32')
