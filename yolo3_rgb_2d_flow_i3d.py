...
resultsPath = './results/yolo_rbg_2d_flow_i3d_'+OF
resultsPath_merge ='./results/merge_yolo_rbg_2d_flow_i3d_'+OF
    ...
    for clip_name_i in clip_name:
        ...
        if len(DirList_jpg)>0:
            ...
            for imgname in DirList_jpg:
                ...
                boxes_rgb  = decode_detections_yolo(Rgb_boxes_raw,Rgb_box_scores_raw,Rgb_box_scores_raw)
                if (len(boxes_rgb)>0):
                ...
                elif (len(boxes_rgb)>0 and len(y_pred_decoded_flow[i])>0  ):
                      boxs_rbg= nms2d(boxes_rgb)
                      boxs_flow= nms2d(y_pred_decoded_flow[i])
                      boxs =merge_score_yolo_ssd(boxs_rbg,boxes_flow)
