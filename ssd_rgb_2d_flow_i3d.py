...
import copy
from models.keras_FFssd300 import ssd_300
...
resultsPath='./results/ssd_rbg_2d_flow_i3d_'+OF
resultsPath_merge='./results/merge_ssd_rbg_2d_flow_i3d_'+OF


model_rgb, predictor_sizes_rgb = ssd_300(image_size=(img_height, img_width, img_channels),
                n_classes=n_classes,
				mode='training',
...
model_flow, predictor_sizes_flow = ssd_I3D_300(input_shape=(num_frame,img_height, img_width, img_channels),
                n_classes=n_classes,
				mode='training',
                l2_regularization=0.00004,#0.0005,
                scales=scales,
...
weights_rgb_path = './weights/ssd/vgg-16_ssd-fcn_ILSVRC-CLS-LOC.h5'
weights_flow_path = './weights/i3d/i3d_inception_rgb_imagenet_and_kinetics_no_top.h5'


if load_prev_model==1:
    model_rgb.load_weights('./weights/ssd/FFssd300_final.h5',by_name=True)
    if OF == 'liteFlow':
        model_flow.load_weights('./weights/i3d/FFssd300_I3D_liteflow_final.h5', by_name=True)
    elif OF == 'Tvl1':
        model_flow.load_weights('./weights/i3d/FFssd300_I3D_Tvl1_final.h5', by_name=True)
else:
    model_rgb.load_weights(weights_rgb_path, by_name=True)
    model_flow.load_weights(weights_flow_path, by_name=True)
    ...
    for clip_name_i in clip_name:
        ...
        if len(DirList_jpg)>0:
            ...
            for imgname in DirList_jpg:
                ...
                X_rgb,X_rgb_orig, y_rgb,XY_scale=inference_single_image(filenames_rgb,model_rgb)
                X_flow,X_flow_orig, y_flow,XY_scale=inference_3d_single_image(filenames_flow,num_frame,model_flow)

                wight=0.5
                y_merge_05 = np.copy(y_rgb)
                y_pred_merge_05=merge_score(y_merge_05,y_flow,wight)
                y_pred_decoded_merge_05 = decode_detections(y_pred_merge_05,
                                               ...
                                               img_width=img_width)
                y_pred_decoded_merge_05=apply_inverse_transforms(y_pred_decoded_merge_05,XY_scale)
                ...
                y_pred_decoded_flow = decode_detections(y_flow,
                                               confidence_thresh=0.5,
                                               iou_threshold=0.2,
                                               top_k=200,
                                               normalize_coords=normalize_coords,
                                               img_height=img_height,
                                               img_width=img_width)
                y_pred_decoded_flow = apply_inverse_transforms(y_pred_decoded_flow,XY_scale)
                ...
                if (len(y_pred_decoded_flow[i])>0):
                    res_flow=frameAP(res_flow,dirnames_rgb,imgname,nms2d(y_pred_decoded_flow[i]),classes,zoom_str, frame_mAP_th)
                else:
                ...
                detection_mAP=[]
                if only_flow:
                    boxs = nms2d(y_pred_decoded_flow[i])
                elif only_rbg:
                    boxs = nms2d(y_pred_decoded_rgb[i])
                elif (len(y_pred_decoded_rgb[i])>0 and len(y_pred_decoded_flow[i])>0 and len(y_pred_decoded_merge_05[i])>0 ):
                      boxs_rbg= nms2d(y_pred_decoded_rgb[i])
                      boxs_flow= nms2d(y_pred_decoded_flow[i])
                      score_rgb=np.mean(boxs_rbg[:,1])
                      score_flow=np.mean(boxs_flow[:,1])
                      if score_flow>score_rgb:
                          boxs =nms2d(y_pred_decoded_merge_05[i],0.01)
                      else:
                          boxs =nms2d(y_pred_decoded_rgb[i])
