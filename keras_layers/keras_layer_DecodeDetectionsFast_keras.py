...
class DecodeDetectionsFast(Layer):
    ...
    def call(self, y_pred, mask=None):
        ...
        class_ids = K.expand_dims(K.to_float(K.argmax(y_pred[...,:-12], axis=-1)), axis=-1)
        # Extract the confidences of the maximal classes.
        confidences = K.reduce_max(y_pred[...,:-12], axis=-1, keep_dims=True)
        ...
        y_pred = K.concat(values=[class_ids, confidences, xmin, ymin, xmax, ymax], axis=-1)
        ...
        def filter_predictions(batch_item):
            ...
            positive_boxes = K.not_equal(batch_item[...,0], 0.0)
            predictions = K.boolean_mask(tensor=batch_item,
                                          mask=positive_boxes)

            def perform_confidence_thresholding():
                # Apply confidence thresholding.
                threshold_met = predictions[:,1] > self.tf_confidence_thresh
                return K.boolean_mask(tensor=predictions,
                                       mask=threshold_met)
            ...
            predictions_conf_thresh = K.cond(K.equal(K.size(predictions), 0), no_positive_boxes, perform_confidence_thresholding)

            def perform_nms():
                scores = predictions_conf_thresh[...,1]

                # `K.image.non_max_suppression()` needs the box coordinates in the format `(ymin, xmin, ymax, xmax)`.
                xmin = K.expand_dims(predictions_conf_thresh[...,-4], axis=-1)
                ymin = K.expand_dims(predictions_conf_thresh[...,-3], axis=-1)
                xmax = K.expand_dims(predictions_conf_thresh[...,-2], axis=-1)
                ymax = K.expand_dims(predictions_conf_thresh[...,-1], axis=-1)
                ...
                maxima = K.gather(params=predictions_conf_thresh,
            ...
            predictions_nms = K.cond(K.equal(K.size(predictions_conf_thresh), 0), no_confident_predictions, perform_nms)
            ...
            def top_k():
                return K.gather(params=predictions_nms,
                                 indices=K.nn.top_k(predictions_nms[:, 1], k=self.tf_top_k, sorted=True).indices,
                                 axis=0)
            def pad_and_top_k():
                padded_predictions = K.pad(tensor=predictions_nms,
                                            paddings=[[0, self.tf_top_k - K.shape(predictions_nms)[0]], [0, 0]],
                                            mode='CONSTANT',
                                            constant_values=0.0)
            ...
            top_k_boxes = K.cond(K.greater_equal(K.shape(predictions_nms)[0], self.tf_top_k), top_k, pad_and_top_k)
