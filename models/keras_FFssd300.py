...
from tensorflow.keras.layers import Input, Lambda, Activation, Conv2D, MaxPooling2D, ZeroPadding2D, Reshape, Concatenate,Conv2DTranspose,Dense,add
...
def ssd_300(image_size,
    ...
    if VGGtrainabel ==False :
        ...
        conv4_3.trainable=False
        pool4.trainable=False
        conv5_1.trainable=False
        conv5_2.trainable=False
    ...
    deconv5_3 = Conv2DTranspose(512, (3, 3),strides=(2, 2),padding='same',name='deconv5_3')(conv5_3)
    ...
    conv4_3_norm_mbox_conf = Conv2D(n_boxes[0] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='conv4_3_norm_mbox_conf')(fusion_conv4_5)
    ...
    conv4_3_norm_mbox_loc = Conv2D(n_boxes[0] * 4, (3, 3), padding='same', kernel_initializer='he_normal', kernel_regularizer=l2(l2_reg), name='conv4_3_norm_mbox_loc')(fusion_conv4_5)
    ...
    if return_predictor_sizes:
        predictor_sizes = np.array([tf.keras.backend.int_shape(conv4_3_norm_mbox_conf)[1:3],
                                    tf.keras.backend.int_shape(fc7_mbox_conf)[1:3],
                                    tf.keras.backend.int_shape(conv6_2_mbox_conf)[1:3],
                                    tf.keras.backend.int_shape(conv7_2_mbox_conf)[1:3],
                                    tf.keras.backend.int_shape(conv8_2_mbox_conf)[1:3],
                                    tf.keras.backend.int_shape(conv9_2_mbox_conf)[1:3]])
