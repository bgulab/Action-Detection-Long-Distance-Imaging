...
from tensorflow.keras.layers import Input, Lambda, Activation, Conv2D, MaxPooling2D, ZeroPadding2D, Reshape, Concatenate,Dense,BatchNormalization,Conv3D,MaxPooling3D,AveragePooling3D,Dropout,GlobalAveragePooling3D,Conv3DTranspose
...
def ssd_300(input_shape,
   ...
   # Determine proper input shape
    ...
    x = Concatenate(axis=channel_axis, name='Mixed_4f')(
        [branch_0, branch_1, branch_2, branch_3])
    ...
    deconv4_b = Conv3DTranspose(512, (3,3,3),strides=(2,2, 2),padding='same',name='deconv4_b')(Mixed_4b)
    t1=int(deconv4_b.shape[1])
    t2=int(Mixed_3c_conv.shape[1])
    strides_num = 1 if t1==t2 else 2

    deconv4_b_pool = MaxPooling3D((strides_num, 1,1), strides=(1, 1, 1), padding='valid', name='MaxPool_deconv4_b_2x2')(deconv4_b)
    ...
    if return_predictor_sizes:
        predictor_sizes = np.array([fusion_conv4_b_3_c_mbox_conf.shape[1:3],
                                    Mixed_4c_mbox_conf.shape[1:3],
                                    Mixed_5c_mbox_conf.shape[1:3],
